...
import {
  getAdminStats,
  getPartialAdminAndTutorUsers,
  getPartialAdminParticipants,
  getPartialAdminTasks,
} from "@/actions/superAdmin/super";
    ...
    <div>
      ...
      <div className="mt-5 flex flex-col gap-10 lg:grid lg:grid-cols-2 xl:gap-x-8 xl:gap-y-7 [&>*]:bg-white [&>*]:border-[1.5px] [&>*]:border-ecx-colors-secondary-blue [&>*]:shadow-[7px_7px_rgba(39,46,75,1)] [&>*]:py-6 [&>*]:px-5">
          ...
          <AdminsList
            ...
            route={"admins"}
            fetch={`adminsxtutors`}
            action={getPartialAdminAndTutorUsers}
          />
          <AdminsList
            className="col-span-1"
            role={"PARTICIPANTS"}
            route={"participants"}
            fetch={`participants`}
            action={getPartialAdminParticipants}
          />
          <TasksTable />
